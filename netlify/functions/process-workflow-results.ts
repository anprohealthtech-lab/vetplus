...
async function handler(req: Request): Promise<Response> {
  ...
  try {
    return await proxyToSupabaseFunction(req, "process-workflow-results");
  } catch (error) {
    console.error("Netlify process-workflow-results proxy error", error);
    return new Response(
      JSON.stringify({ error: "Failed to process workflow results", details: error instanceof Error ? error.message : String(error) }),
      { status: 500, headers: { ...corsHeaders, "Content-Type": "application/json" } },
    );
  }
}
