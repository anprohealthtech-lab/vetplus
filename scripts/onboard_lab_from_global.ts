...
// --- INPUT: Target Lab ID ---
// You can pass this as an argument: npx tsx scripts/onboard_lab_from_global.ts <LAB_ID>
const TARGET_LAB_ID = process.argv[2];

async function main() {
  if (!TARGET_LAB_ID) {
    console.error("❌ Please provide a LAB_ID as an argument.");
    console.error("   Usage: npx tsx scripts/onboard_lab_from_global.ts <LAB_ID>");
    process.exit(1);
  }

  console.log(`🚀 Starting Onboarding for Lab: ${TARGET_LAB_ID}`);
  ...
  // 2. Fetch All Global Tests
  console.log("   🔍 Fetching Global Test Catalog...");
  const { data: globalTests, error: gtError } = await supabase.from('global_test_catalog').select('*');
  ...
  for (const globalTest of globalTests) {
      ...
      if (existingTest) {
          ...
          // C. Link Analytes (Only for NEW tests)
