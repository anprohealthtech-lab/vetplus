...
const PatientPortalSettings: React.FC<PatientPortalSettingsProps> = ({ labId }) => {
  ...
  const handleBulkCreate = async () => {
    ...
    try {
      ...
      if (!res.ok || json.error) {
        throw new Error(json.error || 'Bulk creation failed');
      }
      ...
      setError(err instanceof Error ? err.message : 'An error occurred during bulk creation');
    } finally {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Bulk Action */}
      <div className="border border-dashed border-teal-300 bg-teal-50/50 rounded-xl p-5">
        <div className="flex items-start justify-between gap-4">
          <div>
            ...
            <ul className="text-xs text-teal-700 mt-2 space-y-1">
              ...
              <li>• Returns a CSV with patient name, phone &amp; PIN for WhatsApp dispatch</li>
            </ul>
          </div>
          <button
