-- Use analyte name for descriptive placeholders to avoid code collisions
  ...
  FROM (
    WITH analyte_data AS (
      SELECT
        ...
        COALESCE(
          CASE
            WHEN a.value_type IN ('qualitative', 'semi_quantitative', 'descriptive') THEN
              NULLIF(UPPER(regexp_replace(rv.parameter, '[^A-Za-z0-9]+', '', 'g')), '')
            ELSE
...
COMMENT ON FUNCTION public.get_report_template_context(uuid) IS 'Builds complete context for PDF report generation. Uses analyte name for descriptive placeholders to avoid code collisions.';
