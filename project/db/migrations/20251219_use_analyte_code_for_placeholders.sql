-- Update get_report_template_context to use analytes.code for placeholder generation
-- This ensures placeholders like ANALYTE_WBC_VALUE instead of ANALYTE_WhiteBloodCellCount_VALUE
...
BEGIN
  ...
  -- NEW: Build analyte placeholders using analytes.code field
  -- Pattern: ANALYTE_[Code]_VALUE, ANALYTE_[Code]_UNIT, etc.
...
COMMENT ON FUNCTION public.get_report_template_context(uuid) IS 'Builds complete context for PDF report generation. Uses analytes.code field for ANALYTE_[Code]_VALUE placeholder pattern.';
