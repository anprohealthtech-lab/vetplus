-- Add lab-specific analyte method support and expose method placeholders in report templates

-- 1) Add method columns to lab_analytes (idempotent)
ALTER TABLE public.lab_analytes
ADD COLUMN IF NOT EXISTS method text;

ALTER TABLE public.lab_analytes
ADD COLUMN IF NOT EXISTS lab_specific_method text;

-- 2) Update report template context view to include analyte method
...
SELECT 
    ...
    -- Include test_group_id for proper grouping in PDF generation
    jsonb_agg(
        DISTINCT jsonb_build_object(
            ...
            'method', COALESCE(la.lab_specific_method, la.method),
...
COMMENT ON VIEW public.v_report_template_context IS 'Report template context view - includes test_group_id and method in analytes JSON for proper grouping and placeholders';

-- 3) Update template context function to include ANALYTE_[Code]_METHOD placeholders
...
COMMENT ON FUNCTION public.get_report_template_context(uuid) IS 'Builds complete context for PDF report generation. Adds ANALYTE_[Code]_METHOD placeholders using lab-specific method when available.';
