-- Fix v_order_test_progress_enhanced: is_verified should check result_values.verify_status, not results.status
-- Issue: Panel shows as not verified (0 approved) even when all result_values have verify_status='approved'
-- Root cause: View checks r.status='Approved' instead of rv.verify_status='approved'
...
SELECT 
    ...
    -- FIXED: Only count when result_values has actual non-empty values
    COUNT(DISTINCT CASE 
    ...
    CASE
        ...
        WHEN COUNT(DISTINCT CASE 
            WHEN rv.value IS NOT NULL AND rv.value != '' 
            THEN tga.analyte_id 
        END) < COUNT(DISTINCT tga.analyte_id)
            THEN 'in_progress'
        ELSE 'completed'
    END AS panel_status,

    -- ✅ FIXED: Check verify_status='approved' in result_values instead of results.status='Approved'
    CASE
...
'Order test progress with proper verification status checking result_values.verify_status instead of results.status';
