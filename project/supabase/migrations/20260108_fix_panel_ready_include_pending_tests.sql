-- Fix v_result_panel_status to include test groups that don't have results yet
-- Issue: Orders with multiple tests show as "ready" when only some tests are verified
-- Root cause: View only queries results table, missing tests from order_tests that have no results
...
-- Verify the fix
-- This should show ALL test groups in an order, including those not started
COMMENT ON VIEW v_result_panel_status IS 
'Shows panel readiness for ALL test groups in orders, including those without results yet. Panel is ready only when all analytes are approved.';
