-- ============================================================
-- Fix: AI-resolved reference_range not appearing in PDF/reports
--
-- Root cause: v_report_template_context built the analytes JSON
--   using COALESCE(la.lab_specific_reference_range, a.reference_range)
--   which ignored rv.reference_range entirely.
--
--   When the AI resolves a dynamic/age-specific reference range it
--   writes the result to result_values.reference_range (rv).
--   Because rv.reference_range was never consulted, the PDF always
--   showed the generic analyte-level range instead of the AI value.
--
-- Fix: promote rv.reference_range to the highest priority so that
--   any per-result override (AI-resolved or manually corrected) is
--   used first, then lab-specific, then the analyte default.
-- ============================================================
...
SELECT
    ...
    jsonb_agg(
        DISTINCT jsonb_build_object(
            ...
            -- ★ FIX: rv.reference_range (AI-resolved / per-result override) takes priority
            ...
            -- Sub-grouping fields from test_group_analytes
