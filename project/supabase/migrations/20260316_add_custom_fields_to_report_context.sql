-- ============================================================
-- Migration: Add custom_fields to get_report_template_context RPC
-- Purpose: Include lab-specific custom patient fields (ABHA ID etc.)
--          in the patient object returned by the report context RPC,
--          so they can be used as {{custom_field_key}} placeholders in PDFs.
-- Safe: Only adds a new field to the patient object; no breaking changes.
-- ============================================================
...
BEGIN
  ...
  -- Fetch custom_fields from patients table (not in view)
  SELECT COALESCE(p.custom_fields, '{}'::jsonb)
  INTO patient_custom_fields
  FROM public.patients p
  WHERE p.id = ctx_record.patient_id
  LIMIT 1;
  ...
  RETURN jsonb_strip_nulls(jsonb_build_object(
    ...
    'patient', jsonb_build_object(
      ...
      'custom_fields', patient_custom_fields
    ),
