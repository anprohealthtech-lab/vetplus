...
async function getAIPrompt(
  ...
  try {
    ...
    console.log('  ⚠️  FALLBACK: Using hardcoded default prompt (due to error)');
...
serve(async (req) => {
  ...
  try {
    ...
    const { attachmentId, imageBase64 } = await req.json() as TRFExtractionRequest
    ...
    console.log('  - Using Vision API features: TEXT_DETECTION, DOCUMENT_TEXT_DETECTION');
...
IMPORTANT: Use the IMAGE to verify checkbox states. The OCR text shows what tests are listed, but you must LOOK AT THE IMAGE to see which checkboxes are checked (✓) or unchecked (☐).`;
    ...
    console.log('  - Response preview (first 500 chars):', generatedText.substring(0, 500));
    ...
    try {
      ...
      console.log('  - Patient phone:', extractedData.patientInfo?.phone || 'not found');
      console.log('  - Tests requested:', extractedData.testsRequested?.length || 0);
    } catch (parseError) {
    ...
    if (extractedData.patientInfo?.name || extractedData.patientInfo?.phone) {
      ...
      if (patients && patients.length > 0) {
        ...
        for (const patient of patients) {
          ...
          if (searchName && patientName) {
            if (patientName === searchName) {
              nameMatchScore = 1.0
            } else if (patientName.includes(searchName) || searchName.includes(patientName)) {
              nameMatchScore = 0.7
            } else {
        ...
        if (bestMatch && bestScore > 0.7) {
          ...
          console.log(`✓ Best patient match found: ${bestMatch.name} (${bestMatch.phone}) - Score: ${bestScore.toFixed(2)} (${bestMatch.matchReason})`)
        } else {
          console.log(`⚠ No confident patient match found (best score: ${bestScore.toFixed(2)})`)
        }
      }
    }
      ...
      if (doctorError) {
        ...
        for (const doctor of doctors) {
          ...
          if (docName === searchName) {
            matchScore = 1.0;
            console.log(`   ✓ Exact match: "${doctor.name}" (normalized: "${docName}")`);
          }
          // Substring match (handles "Dr Anand" matching "Dr Anand Priyadarshi")
            ...
            matchScore = 0.8;
            console.log(`   ~ Partial match: "${doctor.name}" (normalized: "${docName}") - Score: ${matchScore}`);
          }
        ...
        if (bestDoctorMatch && bestDoctorScore >= 0.7) {
          ...
          console.log(`✓ Best doctor match: "${bestDoctorMatch.name}" (ID: ${bestDoctorMatch.id}) - Score: ${bestDoctorScore.toFixed(2)}`);
        } else {
          console.log(`⚠ No confident doctor match found (best score: ${bestDoctorScore.toFixed(2)})`);
        }
      } else {
    ...
    if (extractedData.requestedTests && Array.isArray(extractedData.requestedTests)) {
      ...
      if (testGroups && testGroups.length > 0) {
        extractedData.requestedTests = extractedData.requestedTests.map((test: any) => {
          ...
          for (const group of testGroups) {
            ...
            if (groupName === testName || groupCode === testName) {
              ...
              break
            } else if (groupName.includes(testName) || testName.includes(groupName)) {
              const score = 0.8
              if (score > bestScore) {
                bestMatch = group
                bestScore = score
                console.log(`  ~ Partial match: "${group.name}" (score: ${score})`);
              }
            }
          }
          ...
          if (bestMatch) {
            console.log(`✓ Test "${test.testName}" -> Matched "${bestMatch.name}" (${bestScore})`);
          } else {
        ...
        const matchedCount = extractedData.requestedTests.filter((t: any) => t.matched).length;
        console.log(`📊 Test matching complete: ${matchedCount}/${extractedData.requestedTests.length} matched`);
      }
    }
