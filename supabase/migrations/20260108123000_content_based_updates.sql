-- Mass update script for Clinical Interpretations based on *Content Matches*
-- This approach is Test-Name Agnostic. It finds specific HTML Blocks and replaces them.

-- 1. Karyotyping: Replace "Comprehensive cancer karyotyping..." block
...
WHERE gjs_html LIKE '%Comprehensive cancer karyotyping analysis completed%';


-- 2. Routine Package: Replace "Results suggest a comprehensive health overview..." block
UPDATE lab_templates
SET gjs_html = regexp_replace(
    gjs_html, 
...
WHERE gjs_html LIKE '%Results suggest a comprehensive health overview%';


-- 3. Diabetes: Replace "{{clinicalInterpretation}}" block
-- Safety Check: Only apply if HTML *also* contains context keywords like 'Sugar' or 'HbA1c' to avoid replacing unrelated generic templates.
UPDATE lab_templates
SET gjs_html = regexp_replace(
    gjs_html, 
...
WHERE gjs_html LIKE '%{{clinicalInterpretation}}%' 
  AND (gjs_html LIKE '%Sugar%' OR gjs_html LIKE '%HbA1c%' OR gjs_html LIKE '%Glucose%');
