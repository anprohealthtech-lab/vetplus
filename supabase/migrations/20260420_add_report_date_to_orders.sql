-- Migration: Add report_date to orders for admin-controlled report date override
-- When admin back-dates an order, they can optionally set a custom report date.
-- pdfService uses this value if present, otherwise falls back to NOW().

ALTER TABLE public.orders
  ADD COLUMN IF NOT EXISTS report_date DATE NULL;

COMMENT ON COLUMN public.orders.report_date IS
  'Optional admin-set report date override. When set, PDF report prints this date instead of generation timestamp.';

-- Step 1: Recreate view to include report_date
...
COMMENT ON VIEW public.v_report_template_context IS
  'Report template context — includes patient_number (UHID), sample_received_at, collection_center, sample_barcode, and report_date override';


-- Step 2: Update RPC to expose reportDate in meta
