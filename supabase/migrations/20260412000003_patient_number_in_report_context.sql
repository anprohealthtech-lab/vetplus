-- Migration: Add patient_number (UHID) to report context
-- Exposes the new patients.patient_number field (e.g. ML001-P-00142)
-- as {{patientNumber}} placeholder in report templates.

-- Step 1: Recreate view with patient_number
...
COMMENT ON VIEW public.v_report_template_context IS
  'Report template context — includes patient_number (UHID), sample_received_at, and collection_center';


-- Step 2: Update the RPC function to expose patient_number
