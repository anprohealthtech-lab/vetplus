...
CREATE TABLE public.patients (
  ...
  CONSTRAINT patients_patient_auth_id_fkey FOREIGN KEY (patient_auth_id) REFERENCES auth.users(id)
);
CREATE TABLE public.analytes (
  ...
  calculation_result_type text DEFAULT 'numeric'::text CHECK (calculation_result_type = ANY (ARRAY['numeric'::text, 'text'::text])),
...
CREATE TABLE public.test_groups (
  ...
  is_section_only boolean NOT NULL DEFAULT false,
  global_test_catalog_id uuid,
  collection_checklist jsonb NOT NULL DEFAULT '[]'::jsonb,
  sample_condition_options jsonb NOT NULL DEFAULT '[]'::jsonb,
  default_sample_condition text,
  ...
  CONSTRAINT test_groups_global_test_catalog_id_fkey FOREIGN KEY (global_test_catalog_id) REFERENCES public.global_test_catalog(id),
...
CREATE TABLE public.test_group_analytes (
  ...
  lab_id uuid,
  lab_analyte_id uuid,
  report_display_options jsonb NOT NULL DEFAULT '{}'::jsonb,
...
CREATE TABLE public.orders (
  ...
  pdf_callback_status text DEFAULT 'pending'::text,
  pdf_callback_attempts integer DEFAULT 0,
  pdf_callback_last_error text,
  pdf_callback_sent_at timestamp with time zone,
...
CREATE TABLE public.result_values (
  ...
  flag_source text DEFAULT 'auto_numeric'::text CHECK (flag_source IS NULL OR (flag_source = ANY (ARRAY['auto_numeric'::text, 'auto_text'::text, 'auto_rule'::text, 'ai'::text, 'manual'::text, 'inherited'::text, 'analyzer'::text]))),
  ...
  value_type text,
  lab_analyte_id uuid,
  is_hidden_from_report boolean NOT NULL DEFAULT false,
  hidden_reason text,
...
CREATE TABLE public.invoice_items (
  ...
  item_type text NOT NULL DEFAULT 'test'::text CHECK (item_type = ANY (ARRAY['test'::text, 'lab_charge'::text])),
  order_billing_item_id uuid,
  ...
  CONSTRAINT invoice_items_order_billing_item_id_fkey FOREIGN KEY (order_billing_item_id) REFERENCES public.order_billing_items(id)
);
...
CREATE TABLE public.labs (
  ...
  email_domain text,
  pdf_layout_settings jsonb DEFAULT '{"scale": 1.0, "margins": {"top": 180, "left": 20, "right": 20, "bottom": 150}, "mediaType": "screen", "paperSize": "A4", "orientation": "portrait", "footerHeight": 80, "headerHeight": 90, "printOptions": {"calcMarker": "cal", "flagSymbol": "before", "qrPosition": "bottom_left", "baseFontSize": 14, "flagAsterisk": false, "testNameBold": false, "boldAllValues": false, "showFlagLegend": false, "basicColumnWidths": {"sibling": [30, 14, 8, 16, 16, 16], "standard": [36, 24, 12, 28]}, "testNameAlignment": "left", "boldAbnormalValues": true, "qrHorizontalOffset": 0, "sectionHeaderInline": true, "flagAsteriskCritical": false, "testGroupTitlePosition": "above_headers_center"}, "resultColors": {"low": "#000000", "high": "#dc2626", "normal": "#16a34a", "enabled": true}, "headerTextColor": "white", "printBackground": true, "displayHeaderFooter": true}'::jsonb,
  ...
  default_template_style text DEFAULT 'basic'::text CHECK (default_template_style = ANY (ARRAY['beautiful'::text, 'classic'::text, 'basic'::text])),
  ...
  show_dual_signatory boolean NOT NULL DEFAULT false,
...
CREATE TABLE public.reports (
  ...
  compact_ecopy_url text,
  compact_ecopy_generated_at timestamp with time zone,
...
CREATE TABLE public.lab_analytes (
  ...
  ai_prompt_override text,
  group_ai_mode text,
  attached_test_group_count integer NOT NULL DEFAULT 0,
...
CREATE TABLE public.accounts (
  ...
  price_master_id uuid,
  credit_used numeric NOT NULL DEFAULT 0 CHECK (credit_used >= 0::numeric),
  credit_balance numeric DEFAULT (credit_limit - credit_used),
  ...
  CONSTRAINT accounts_price_master_id_fkey FOREIGN KEY (price_master_id) REFERENCES public.price_masters(id)
);
...
CREATE TABLE public.lab_branding_assets (
  id uuid NOT NULL DEFAULT gen_random_uuid(),
  lab_id uuid NOT NULL,
  asset_type character varying NOT NULL CHECK (asset_type::text = ANY (ARRAY['header'::character varying, 'footer'::character varying, 'watermark'::character varying, 'logo'::character varying, 'letterhead'::character varying, 'front_page'::character varying, 'last_page'::character varying]::text[])),
...
CREATE TABLE public.result_section_content (
  ...
  cascading_selections jsonb DEFAULT '{}'::jsonb,
...
CREATE TABLE public.analyte_dependencies (
  ...
  created_at timestamp with time zone DEFAULT now(),
  lab_id uuid,
  calculated_lab_analyte_id uuid,
  source_lab_analyte_id uuid,
  ...
  CONSTRAINT analyte_dependencies_calculated_lab_analyte_id_fkey FOREIGN KEY (calculated_lab_analyte_id) REFERENCES public.lab_analytes(id),
  CONSTRAINT analyte_dependencies_source_lab_analyte_id_fkey FOREIGN KEY (source_lab_analyte_id) REFERENCES public.lab_analytes(id)
);
...
CREATE TABLE public.lab_notification_settings (
  ...
  auto_send_registration_to_doctor boolean DEFAULT false,
  ...
  created_at timestamp with time zone DEFAULT now(),
  updated_at timestamp with time zone DEFAULT now(),
  name_case_format text DEFAULT 'proper'::text CHECK (name_case_format = ANY (ARRAY['proper'::text, 'upper'::text])),
  doctor_report_recipients text DEFAULT 'doctor_only'::text CHECK (doctor_report_recipients = ANY (ARRAY['doctor_only'::text, 'both'::text])),
  auto_send_loyalty_points boolean DEFAULT false,
  auto_send_loyalty_redemption boolean DEFAULT false,
...
CREATE TABLE public.notification_queue (
  ...
  recipient_name text,
  recipient_id uuid,
  trigger_type text NOT NULL CHECK (trigger_type = ANY (ARRAY['report_ready'::text, 'invoice_generated'::text, 'order_registered'::text, 'payment_reminder'::text, 'loyalty_points_earned'::text, 'loyalty_points_redeemed'::text])),
  order_id uuid,
  report_id uuid,
  invoice_id uuid,
  template_id uuid,
...
CREATE TABLE public.test_mappings (
  ...
  mapping_type text NOT NULL DEFAULT 'result_analyte'::text CHECK (mapping_type = ANY (ARRAY['order_service'::text, 'result_analyte'::text, 'result_section'::text, 'specimen_mode'::text, 'histogram'::text, 'flag'::text, 'control'::text])),
  direction text NOT NULL DEFAULT 'bidirectional'::text CHECK (direction = ANY (ARRAY['outbound'::text, 'inbound'::text, 'bidirectional'::text])),
  ...
  CONSTRAINT test_mappings_lab_analyte_id_fkey FOREIGN KEY (lab_analyte_id) REFERENCES public.lab_analytes(id),
  CONSTRAINT test_mappings_section_id_fkey FOREIGN KEY (section_id) REFERENCES public.lab_template_sections(id)
);
