-- Migration: Fix v_report_template_context view UUID cast error
-- Date: 2025-11-16
-- Issue: sample_collected_by is text (email) not UUID, causing cast error
-- Fix: Join users table on email instead of trying to cast to UUID

-- Drop and recreate view to fix column structure
...
-- FIXED: sample_collected_by is text (email), match on email field instead of UUID cast
