-- Use test group methodology for {{technique}} and {{methodology}} placeholders
...
DECLARE
  ...
  test_group_methodology text := NULL;
BEGIN
  ...
  SELECT string_agg(DISTINCT tg.methodology, ', ')
  INTO test_group_methodology
  FROM public.test_groups tg
  WHERE tg.id = ANY(ctx_record.test_group_ids)
    AND tg.methodology IS NOT NULL
    AND btrim(tg.methodology) <> '';
  ...
  base_placeholders := jsonb_build_object(
    ...
    'technique', coalesce(test_group_methodology, ''),
    'methodology', coalesce(test_group_methodology, '')
  );
...
COMMENT ON FUNCTION public.get_report_template_context(uuid) IS
  'Builds complete context for PDF report generation. Adds section images and exposes technique/methodology placeholders.';
