...
import DetailedApprovalModal from '../components/Results/DetailedApprovalModal';
...
export default function ResultsVerification() {
  ...
  const [selectedTestForModal, setSelectedTestForModal] = useState<TestGroup | null>(null);
  ...
  const loadData = useCallback(async () => {
    try {
      ...
      // 1. Try preferred consolidated view first (verification_queue). If it doesn't exist, fall back.
      ...
      if (viewAttempt.error) {
        resultsError = viewAttempt.error;
        console.warn('verification_queue view unavailable, falling back to base tables. Error:', resultsError?.message);
      } else {
        results = viewAttempt.data || [];
      }

      // (Optional) Fallback path could query raw tables. For now just proceed if results is available.
      if (!results) {
        // Graceful empty state rather than throwing; prevents hard failure if view missing temporarily.
        results = [];
      }

      // 2. Fetch values for each result (keep existing RPC; tolerate missing function).
      const resultsWithValues = await Promise.all(
        results.map(async (result) => {
          try {
            ...
            console.warn('get_result_values RPC failed for', result.result_id, e);
            return { ...result, values: [] };
          }
        })
      );

      // 3. Group results by composite key (order + test). Use groupKey as stable id to avoid duplicates.
      ...
      for (const result of resultsWithValues) {
        ...
        if (!groupedTests.has(groupKey)) {
          groupedTests.set(groupKey, {
            id: groupKey, // stable composite id
      ...
      // 4. Stats (optional) – don't fail whole load if view absent.
      try {
        const { data: stats, error: statsError } = await supabase
          ...
          .single();
        if (statsError) {
          console.warn('view_verification_stats not available:', statsError.message);
        } else {
  ...
  // Centralized verification dispatcher with bulk RPC fallback
  const performVerification = async (resultIds: string[], status: 'verified' | 'rejected' | 'needs_clarification', notes?: string) => {
    if (!resultIds.length) return;
    const action = status === 'verified' ? 'approve' : status === 'rejected' ? 'reject' : 'clarify';
    const comment = notes || verificationNotes;
    ...
    try {
      if (resultIds.length > 1) {
        ...
        console.warn('bulk_verify_results failed, falling back to per-result:', bulkError.message);
      }
    } catch (e) {
      console.warn('bulk_verify_results RPC unavailable, falling back.', e);
    }

    // Fallback: individual RPC calls
    try {
      ...
      console.error('Error during per-result verification RPCs:', e);
      alert('Error verifying result(s). Please try again.');
    }
  };

  const verifyResult = async (resultId: string, status: 'verified' | 'rejected' | 'needs_clarification', notes?: string) => {
    await performVerification([resultId], status, notes);
  };
  ...
  const bulkVerifyResults = async (status: 'verified' | 'rejected' | 'needs_clarification') => {
    ...
    await performVerification(Array.from(selectedResultIds), status);
  ...
  const verifyEntireTest = async (testGroup: TestGroup, status: 'verified' | 'rejected' | 'needs_clarification') => {
    const resultIds = testGroup.results.map(r => r.result_id);
    await performVerification(resultIds, status);
  };
  ...
  const openDetailedModal = (testGroup: TestGroup) => {
    setSelectedTestForModal(testGroup);
    setShowDetailedModal(true);
  };

  // Handle modal approval actions
  const handleModalApprove = async (resultId: string, notes: string) => {
    try {
  ...
  const handleModalReject = async (resultId: string, notes: string) => {
    try {
      const { error } = await supabase
        .rpc('verify_result', {
          p_result_id: resultId,
  ...
  const handleModalClarify = async (resultId: string, notes: string) => {
    try {
      const { error } = await supabase
        .rpc('verify_result', {
          p_result_id: resultId,
      ...
      alert('Error requesting clarification. Please try again.');
    }
  };
  ...
  return (
    <div className="bg-gray-50">
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      <div className="flex items-center space-x-2">
                        ...
                        <div className="flex space-x-1">
                          <button
                            onClick={() => openDetailedModal(testGroup)}
      ...
      {selectedTestForModal && (
        <DetailedApprovalModal
          isOpen={showDetailedModal}
          onClose={() => setShowDetailedModal(false)}
          testResult={{
            id: selectedTestForModal.id,
            order_id: selectedTestForModal.order_id,
            patient_id: selectedTestForModal.patient_id,
            patient_name: selectedTestForModal.patient_name,
            test_name: selectedTestForModal.test_name,
            verification_status: selectedTestForModal.verification_status,
            entered_by: selectedTestForModal.entered_by,
            entered_date: selectedTestForModal.entered_date,
            technician_notes: selectedTestForModal.technician_notes,
            critical_flag: selectedTestForModal.critical_flag,
            priority_level: 1, // Default priority level
            sample_id: selectedTestForModal.sample_id,
            order_priority: selectedTestForModal.order_priority,
            order_date: selectedTestForModal.order_date,
            physician_name: selectedTestForModal.physician_name,
            full_patient_name: selectedTestForModal.patient_name,
            gender: selectedTestForModal.gender,
            patient_code: selectedTestForModal.patient_code,
            age: selectedTestForModal.age,
            hours_since_entry: selectedTestForModal.hours_since_entry,
            results: selectedTestForModal.results.map(result => ({
          ...
          onRequestClarification={handleModalClarify}
        />
      )}
    </div>
  );
