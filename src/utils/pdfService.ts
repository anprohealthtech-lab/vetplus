...
import {
  ensureReportRegions,
  extractReportRegions,
} from "./reportTemplateRegions";
import {
  generateReportExtrasHtml,
  getReportExtrasForOrder,
} from "./reportExtrasService";
import { determineFlag, flagToDisplayString } from "./flagDetermination";
...
export { analyzePDFComplexity, warmupPuppeteer };
...
const sanitizeRegionHtml = (html?: string | null): string | null => {
  ...
  const cleaned = trimmed
    .replace(
      /<[^>]*class=["'][^"']*report-region-placeholder[^"']*["'][^>]*>([\s\S]*?)<\/[^>]+>/gi,
      (match, inner) => {
  ...
  const hasVisualElements =
    /<\s*(img|svg|picture|table|canvas|iframe|video|div|span)[^>]*>/i.test(
      workingHtml,
    );
  ...
  const isNamedPlaceholder = textLower === "header content" ||
...
const buildDefaultTemplateContext = (): Record<string, string> => ({
  ...
  interpretationNotes:
    "All parameters are within expected ranges for the provided demographic profile.",
  hemoglobin: "14.2",
  hematocrit: "42",
  rbcCount: "5.10",
  mcv: "88",
...
export const buildSampleTemplateContext = (
...
const buildContextFromReportTemplate = (
  ...
  const derived: Record<string, any> = {
    ...
    dateOfBirth: patient?.dateOfBirth
      ? normalizeDateValue(patient.dateOfBirth)
      : "",
    patientRegistrationDate: patient?.registrationDate
      ? normalizeDateValue(patient.registrationDate)
      : "",
    sampleId: order?.sampleId ?? "",
    sampleCollectedAt: order?.sampleCollectedAt
      ? normalizeDateValue(order.sampleCollectedAt)
      : "",
  ...
  return Object.fromEntries(
    Object.entries(derived).filter(([, value]) =>
...
const normalizeCustomCss = (css?: string | null): string => {
  ...
  let normalized = css.replace(/\u00a0/g, " ").replace(
    /undefined\s*:\s*undefined;?/gi,
    "",
  ).trim();
...
const renderTemplateWithContext = (
  html: string,
...
const buildPreviewDocument = (
  ...
  const resolvedHeader = sanitizeRegionHtml(headerHtml) ?? fallbackHeader ??
    DEFAULT_HEADER_HTML;
  const resolvedFooter = sanitizeRegionHtml(footerHtml) ?? fallbackFooter ??
    DEFAULT_FOOTER_HTML;
...
const buildPdfBodyDocument = (bodyHtml: string, customCss: string): string => {
  ...
  const styleTagsInHtml =
    htmlDocument.match(/<style[^>]*>[\s\S]*?<\/style>/gi) || [];
...
export const buildReportHtmlBundle = (
...
const buildReportFilenameBase = (
  reportData: ReportData,
  isDraft: boolean,
): string => {
  const safePatient = (reportData.patient?.name || "Patient").replace(
    /\s+/g,
    "_",
  );
...
const pollPdfCoJob = async (
  ...
  for (let attempt = 0; attempt < maxAttempts; attempt += 1) {
    ...
    if (!response.ok) {
      throw new Error(
        `PDF.co job status error: ${response.status} ${response.statusText}`,
      );
    }
    ...
    const errorMessage = statusPayload.message || statusPayload.error ||
      `Job returned status: ${status}`;
...
export const sendHtmlToPdfCo = async (
  ...
  if (!response.ok) {
    throw new Error(
      `PDF.co API error: ${response.status} ${response.statusText}`,
    );
  }
  ...
  if (result.url) {
    ...
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
      try {
        ...
        console.warn(
          `⏳ PDF URL not ready (attempt ${attempt}/${maxRetries}), status: ${headCheck.status}`,
        );
      } catch (fetchError) {
        console.warn(
          `⏳ PDF URL check failed (attempt ${attempt}/${maxRetries}):`,
          fetchError,
        );
      }
    ...
    console.warn(
      "⚠️ PDF URL not verified after retries, returning anyway:",
      result.url,
    );
    return result.url;
  }
...
const sendPrintHtmlToPdfCo = async (
  ...
  console.log(
    "  Print HTML has proper structure:",
    printHtml.includes("limsv2-report--print"),
  );
  ...
  if (options.context) {
    ...
    const safeSectionContent = Object.fromEntries(
      Object.entries(sectionContent)
        ...
        .map((
          [key, value],
        ) => [key, new nunjucks.runtime.SafeString(String(value))]),
    );
    renderContext = {
      ...renderContext,
      ...derivedContext,
  ...
  for (const match of matches) {
    ...
    try {
      ...
      if (base64Src) {
        ...
        console.log(
          `Converted image URL to base64: ${imageUrl.substring(0, 50)}...`,
        );
      }
    } catch (error) {
...
export const generateTemplatePreviewPDF = async (
  ...
  const hasMultipleTestGroups = context?.testGroupIds &&
    context.testGroupIds.length > 1;

  if (hasMultipleTestGroups && allTemplates && allTemplates.length > 0) {
    console.log(
      "🔀 Preview: Multi-template mode for",
      context!.testGroupIds!.length,
      "test groups",
    );
    ...
    const reportData: ReportData = {
      patient: {
        ...
        age: typeof context!.patient?.age === "number"
          ? context!.patient.age
          : 0,
  ...
  const filename = `${
    template.template_name?.replace(/\s+/g, "_") || "Template"
  }_Preview.pdf`;
...
const resolveBrandingDefaultsFromContext = (
  context?: ReportTemplateContext | null,
): LabBrandingHtmlDefaults => {
  if (!context) {
    return {};
  }

  const placeholders = (context.placeholderValues ?? {}) as Record<
    string,
    unknown
  >;
...
const resolveReportBrandingDefaults = (
  reportData: ReportData,
): LabBrandingHtmlDefaults => {
  const overrides = (reportData.placeholderOverrides ?? {}) as Record<
    string,
    unknown
  >;
  ...
  const contextDefaults = resolveBrandingDefaultsFromContext(
    reportData.templateContext,
  );
...
const resolveBrandingDefaultsFromOptions = (
  options: TemplateRenderOptions = {},
): LabBrandingHtmlDefaults => {
...
const buildTestResultsFromAnalytes = (
  ...
  const filtered = includeUnapproved
    ? analytes
    : analytes.filter((row) =>
      !row.verify_status || row.verify_status === "approved"
    );
  ...
  return filtered.reduce<TestResult[]>((results, row) => {
    ...
    if (!flag) {
      ...
      const referenceRange =
        patientGender?.toLowerCase() === "male" && row.reference_range_male
...
export const selectTemplateForContext = (
  ...
  const testGroupIds = Array.isArray(context.testGroupIds)
  ...
  if (analyteNames.size) {
    const byName = templatesWithHtml.find((tpl) => {
      ...
      return Array.from(analyteNames).some((name) =>
...
const resolveReportType = (
  ...
  const preferredValue = placeholders["report_type"] ??
...
const resolveInterpretation = (
  ...
  const placeholders = context.placeholderValues ?? {};
  const interpretationValue = placeholders["report_interpretation"] ??
...
const generateUniversalHTMLTemplate = (
  data: ReportData,
  isDraft = false,
): string => {
...
<body>
  ...
  <div class="info-grid">
    ...
    report.collectionDate
      ? new Date(report.collectionDate).toLocaleString("en-IN", { day: "2-digit", month: "2-digit", year: "numeric", hour: "2-digit", minute: "2-digit", hour12: true })
      : ""
  }</span>
      </div>
      <div class="info-item">
  ...
  <div class="results-section">
    ...
    testResults.map((result) => `
      ...
      result.flag
        ...
        ${testResults.some(r => r.isCalculated) ? `
          <tr><td colspan="5" style="font-size:10px;color:#9ca3af;padding:4px 8px;border:none;font-style:italic;">*calc – Calculated parameter</td></tr>
        ` : ""}
      </tbody>
    </table>
  </div>
 ...
 */
const groupAnalytesByTestGroup = (
  analytes: ReportTemplateAnalyteRow[],
): Map<string, ReportTemplateAnalyteRow[]> => {
...
const renderMultipleTestGroupTemplates = async (
  ...
  console.log(
    `📋 Found ${analytesByGroup.size} test group(s) in order ${context.orderId}`,
  );

  // If only one test group, use the standard single-template rendering
  if (analytesByGroup.size === 1) {
    const template = reportData.labTemplateRecord ||
      selectTemplateForContext(templates, context);
    if (template?.gjs_html) {
  ...
  for (const [testGroupId, groupAnalytes] of analytesByGroup.entries()) {
    ...
    if (testGroupId !== "ungrouped") {
      groupTemplate = templates.find((t) =>
        t.test_group_id === testGroupId && t.gjs_html
      ) || null;
    }
    ...
    if (!groupTemplate) {
      groupTemplate = templates.find((t) => t.is_default && t.gjs_html) ||
        templates.find((t) => t.gjs_html) || null;
    }
...
const injectWatermarkIfEnabled = async (
  html: string,
  labId: string,
): Promise<string> => {
  try {
    const { data: labSettings, error } = await supabase
      .from("labs")
      .select(
        "watermark_enabled, watermark_image_url, watermark_opacity, watermark_position, watermark_size, watermark_rotation",
      )
      .eq("id", labId)
      .single();

    if (
      error || !labSettings || !labSettings.watermark_enabled ||
      !labSettings.watermark_image_url
    ) {
    ...
    const maxWidth =
      sizeMap[labSettings.watermark_size as keyof typeof sizeMap] || "60%";
    ...
    const positionMap: Record<string, string> = {
      ...
      "repeat": "top:0;left:0;width:100%;height:100%;background-image:url(" +
        labSettings.watermark_image_url +
        ");background-repeat:repeat;background-size:30%;opacity:" +
        labSettings.watermark_opacity,
    };

    const isRepeat = labSettings.watermark_position === "repeat";
    const positionStyle = positionMap[labSettings.watermark_position] ||
      positionMap["center"];
    ...
    const watermarkHtml = isRepeat
      ...
      : `<img src="${labSettings.watermark_image_url}" style="position:absolute;${positionStyle};${
        rotation !== 0
          ? `transform:translate(-50%, -50%) rotate(${rotation}deg);`
          : ""
      }max-width:${maxWidth};height:auto;opacity:${labSettings.watermark_opacity};z-index:1;pointer-events:none;" alt="Watermark" />`;
    ...
    if (bodyMatch) {
      ...
      return html.slice(0, insertIndex) + watermarkHtml +
 ...
 */
const injectSectionContent = async (
  html: string,
  context?: ReportTemplateContext,
): Promise<string> => {
...
const prepareReportHtml = async (
  ...
  if (hasMultipleTestGroups && allTemplates && allTemplates.length > 0) {
    console.log(
      `🔀 Detected ${
        context!.testGroupIds!.length
      } test groups, attempting multi-template merge`,
    );
    ...
    try {
      ...
      console.error(
        "❌ Multi-template merge failed, falling back to single template:",
        error,
      );
  ...
  if (!finalHtml && reportData.labTemplateRecord?.gjs_html && context) {
    ...
    if (reportData.labTemplateRecord.is_interpretation_only) {
      console.log(
        "📝 Template is interpretation only. Appending to universal template.",
      );
      const baseHtml = generateUniversalHTMLTemplate(reportData, isDraft);
      finalHtml = baseHtml.replace(
        "</body>",
        `\n<div class="interpretation-only-template" style="margin-top: 24px;">\n${bundle.bodyHtml}\n</div>\n</body>`,
      );
    } else {
  ...
  if (context?.orderId) {
    try {
      ...
      if (
        reportExtras &&
        (reportExtras.trend_charts?.length || reportExtras.clinical_summary)
      ) {
        console.log(
          `📊 Injecting report extras for order ${context.orderId}:`,
          {
...
export const generatePDFWithAPI = async (
  ...
  console.log(
    "Generating PDF with PDF.co API...",
    isDraft ? "(DRAFT)" : "(FINAL)",
  );
  const ready = prepared
    ? (prepared instanceof Promise ? await prepared : prepared)
    : await prepareReportHtml(reportData, isDraft);
  ...
  if (reportData.templateContext?.labId) {
    const { data: labDefaults } = await supabase
      .from("labs")
      .select(
        "default_report_header_html, default_report_footer_html, pdf_layout_settings",
      )
    ...
    if (pdfSettings) {
      console.log(
        "📄 Using PDF settings from database (lab-level):",
        pdfSettings,
      );
    }
  }
  ...
  const headerHeight = pdfSettings?.headerHeight
    ? `${pdfSettings.headerHeight}px`
    : "90px";
  const footerHeight = pdfSettings?.footerHeight
...
export const generatePDFWithBrowser = async (
  reportData: ReportData,
): Promise<string> => {
...
export const updateReportWithPDFInfo = async (
  orderId: string,
  pdfUrl: string,
  reportType: string = "final",
): Promise<void> => {
  ...
  try {
    ...
    if (reportType === "final") {
      ...
      if (report) {
        notificationTriggerService.triggerReportReady(
          orderId,
          report.id,
          pdfUrl,
          report.lab_id,
          ...
          .catch((err) =>
            console.error("Error triggering report ready notification:", err)
          );
      }
    }
  } catch (error) {
...
export async function generateAndSavePDFReportWithProgress(
  ...
  console.log(
    "generateAndSavePDFReportWithProgress called for order:",
    orderId,
    "isDraft:",
    isDraft,
  );
  ...
  try {
    ...
    if (!needsRegeneration && existingReport?.pdf_url) {
      console.log(
        `${reportType.toUpperCase()} PDF already exists and is current:`,
        existingReport.pdf_url,
      );
      onProgress?.(`Validating existing ${reportType} PDF...`, 20);

      try {
        const response = await fetch(existingReport.pdf_url, {
          method: "HEAD",
        });
        if (response.ok) {
    ...
    const preparedHtml = await prepareReportHtml(
      reportData,
      isDraft,
      allTemplates,
    );
      ...
      try {
        ...
        const puppeteerUrl = await Promise.race([
          ...
          new Promise<string>((_, reject) =>
            setTimeout(
              () => reject(new Error("Puppeteer timeout: 2s exceeded")),
              2000,
            )
          ),
        ]);
        ...
        if (puppeteerUrl) {
          ...
          if (!isDraft) {
            ...
            try {
              ...
              const printPreparedHtml = await prepareReportHtml(
                reportData,
                isDraft,
                allTemplates,
                true,
              ...
              const printUrl = await Promise.race([
                ...
                new Promise<string>((_, reject) =>
                  setTimeout(
                    () => reject(new Error("Print PDF timeout: 2s exceeded")),
                    2000,
                  )
                ),
              ]);
              ...
              console.error(
                "⚠️ Print PDF generation failed (non-critical):",
                printError,
              );
        ...
        console.warn(
          `⚠️ Puppeteer generation failed (${failTime}ms), falling back to PDF.co:`,
          puppeteerError,
        );
    ...
    try {
      ...
      const mainPdfPromise = generatePDFWithAPI(
        reportData,
        isDraft,
        preparedHtml,
      );
      ...
      if (!isDraft) {
        const printPreparedHtml = prepareReportHtml(
          reportData,
          isDraft,
          allTemplates,
          true,
      ...
      if (
        pdfUrl &&
        (pdfUrl.includes("pdf.co") ||
          pdfUrl.includes("s3.us-west-2.amazonaws.com"))
      ) {
        ...
        try {
          ...
          console.log(
            "✅ Main PDF successfully downloaded:",
            pdfBlob.size,
            "bytes",
          );
          ...
          console.warn(
            "⚠️ Robust download failed, trying standard method:",
            downloadError,
          );
          ...
          if (response.ok) {
            ...
            throw new Error(
              `Standard download failed: ${response.status} ${response.statusText}`,
            );
          }
        }
        ...
        if (printPdfPromise) {
          ...
          try {
            ...
            try {
              printBlob = await downloadLargePDFWithProgress(
                printPdfUrl,
                onProgress,
              );
            } catch (printDownloadError) {
              console.warn(
                "Print PDF robust download failed, attempting standard fetch...",
                printDownloadError,
              );
              ...
              if (response.ok) {
                ...
                throw new Error(
                  `Standard print PDF download failed: ${response.status} ${response.statusText}`,
                );
              }
            }
            ...
            if (printBlob) {
              ...
              const printStorageUrl = await savePDFToStorage(
                printBlob,
                orderId,
                "print",
              );
            ...
            console.error(
              "⚠️ Print PDF generation/save failed (non-critical):",
              printError,
            );
          }
        }
    ...
    const storageUrl = await savePDFToStorage(
      pdfBlob,
      orderId,
      isDraft ? "draft" : "final",
    );
...
export const downloadLargePDFWithProgress = async (
  ...
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    try {
      ...
      onProgress?.(
        `Downloading... (Attempt ${attempt}/${maxRetries})`,
        40 + (attempt - 1) * 10,
      );
      ...
      if (expectedLength > 0 && receivedLength !== expectedLength) {
        console.warn(
          `⚠️ Size mismatch: expected ${expectedLength}, got ${receivedLength}`,
        );
        if (receivedLength < expectedLength * 0.9) {
          throw new Error(
            `Incomplete download: ${receivedLength}/${expectedLength} bytes`,
          );
        }
      }
      ...
      if (attempt === maxRetries) {
        const errorMessage = error instanceof Error
          ...
          : String(error);
        throw new Error(
          `Failed to download PDF after ${maxRetries} attempts: ${errorMessage}`,
        );
      }
...
export async function generateAndSavePDFReport(
  ...
  try {
    ...
    if (!isDraft) {
      ...
      try {
        const printResult = await generatePrintPDFWithAPI(
          reportData,
          preparedHtml,
        );
        const printPdfUrl = printResult.url;
        ...
        try {
          ...
          console.warn(
            "Print PDF robust download failed, trying standard fetch...",
            printDownloadError,
          );
...
export async function viewPDFReport(
  orderId: string,
  reportData: ReportData,
): Promise<string | null> {
...
export async function downloadPDFReport(
  ...
  try {
    ...
    link.download = `Report_${
      reportData.patient.name.replace(/\s+/g, "_")
    }_${orderId}.pdf`;
    link.target = "_blank";
...
export const generateSampleReportData = (
  template: LabTemplate = defaultLabTemplate,
): ReportData => {
  return {
    patient: {
      name: "Ravi Mehta",
...
export const downloadLargePDF = async (
  url: string,
  maxRetries: number = 3,
): Promise<Blob> => {
  console.log("🔄 Downloading large PDF from:", url);
    ...
    try {
      ...
      while (true) {
        ...
        if (expectedLength > 0) {
          ...
          console.log(
            `📈 Download progress: ${progress}% (${receivedLength}/${expectedLength} bytes)`,
          );
        }
      }
...
export const debugPDFPipeline = async (): Promise<void> => {
  ...
  try {
    ...
    if (supabaseResponse.ok) {
      ...
      console.log(
        "📏 Downloaded from Supabase size:",
        supabaseArrayBuffer.byteLength,
      );
      ...
      const downloadedFirstBytes = new Uint8Array(
        supabaseArrayBuffer.slice(0, 10),
      );
      const downloadedHeader = String.fromCharCode(...downloadedFirstBytes);
      console.log(
        "📄 Downloaded PDF header:",
        JSON.stringify(downloadedHeader),
      );

      // Test if downloaded version works
      const downloadedBlob = new Blob([supabaseArrayBuffer], {
        type: "application/pdf",
      });
      ...
      for (
        let i = 0;
        i < Math.min(originalBytes.length, downloadedBytes.length);
        i++
      ) {
        if (originalBytes[i] !== downloadedBytes[i]) {
          ...
          if (differences <= 10) {
            console.log(
              `Byte difference at ${i}: ${originalBytes[i]} vs ${
                downloadedBytes[i]
              }`,
            );
          }
    ...
    console.log(
      "✅ Debug pipeline completed. Check the opened tabs to compare results.",
    );
  } catch (error) {
 ...
 */
export const getCachedPDFBundle = (
  orderId: string,
...
export const regeneratePDFWithSettings = async (
  ...
  if (showHeaders) {
    ...
    if (rawHeader && options.headerHeight) {
      finalHeaderHtml = constrainHeaderFooterImages(
        rawHeader,
    ...
    if (rawFooter && options.footerHeight) {
      finalFooterHtml = constrainHeaderFooterImages(
        rawFooter,
