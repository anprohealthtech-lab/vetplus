...
-- To make this migration safe to run as a single batch, we avoid CONCURRENTLY here.
-- If you prefer concurrent index creation, run these two CREATE INDEX statements
-- separately in their own query tabs.
CREATE INDEX IF NOT EXISTS idx_users_whatsapp_sync_status 
...
-- Ensure system_config table exists (needed for settings below)
